...
//		nhomMau();
	...
	public Change_infoView(String id) {
		setLayout(null);
	...
	public void button_return(String id) {
	...
	public void birthday(String id) {
		...
		tf_ngaySinh = new JTextField(InforController.getInstance(id).ngaySinh());
	...
	public void ngheNghiep(String id) {
		...
		tf_ngheNghiep = new JTextField(InforController.getInstance(id).ngheNghiep());
	...
	public void button_save(String id) {
		JButton bt_save = new JButton("Lưu thay đổi");
		bt_save.setFont(new Font("#9Slide01 Tieu de ngan", Font.PLAIN, 18));
		bt_save.setBounds(600, 600, 178, 29);
		add(bt_save);
		bt_save.addActionListener(new ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				InforController.getInstance(id).set_Ten(tf_name.getText());
				InforController.getInstance(id).set_sdt(tf_sdt.getText());
				InforController.getInstance(id).set_ngaySinh(tf_ngaySinh.getText());
				InforController.getInstance(id).set_nhomMau(tf_nhomMau.getText());
				InforController.getInstance(id).set_diaChi(tf_diaChi.getText());
				InforController.getInstance(id).set_CCCD(tf_CCCD.getText());
				InforController.getInstance(id).set_ngheNghiep(tf_ngheNghiep.getText());
				InforController.getInstance(id).set_noiCongTac(tf_noiCongTac.getText());
				System.out.println(InforController.getInstance(id).name());
				System.out.println(InforController.getInstance(id).sdt());
				System.out.println(InforController.getInstance(id).ngaySinh());
				System.out.println(InforController.getInstance(id).nhomMau());
				System.out.println(InforController.getInstance(id).diaChi());
				System.out.println(InforController.getInstance(id).CCCD());
				System.out.println(InforController.getInstance(id).ngheNghiep());
				System.out.println(InforController.getInstance(id).noiCongTac());
				JOptionPane.showMessageDialog(null, "Lưu thành công");
			}
		});
	}

	public void noiCongTac(String id) {
		...
		tf_noiCongTac = new JTextField(InforController.getInstance(id).noiCongTac());
	...
	public void nhomMau(String id) {
		...
		tf_nhomMau = new JTextField(InforController.getInstance(id).nhomMau());
